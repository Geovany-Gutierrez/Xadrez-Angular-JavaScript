...
export class ChessBoard {
  ...
  private areCoordsValid(x: number, y: number): boolean {
    return x >= 0 && x < this.chessBoardSize && y < this.chessBoardSize;
  }
  ...
  private isPositionSafeAfterMove(
    ...
    //evitar que peças sejam colocadas onde ja ha peças da mesma cor
    ...
    for (let x = 0; x < this.chessBoardSize; x++) {
      for (let y = 0; y < this.chessBoardSize; y++) {
        ...
        for (const { x: dx, y: dy } of piece.direction) {
          ...
          //restringir movimentos dos peoes em certas direçoes
          if (piece instanceof Pawn) {
            //nao é possivel mover o peao duas casas se tiver alguem na frente
            if (dx === 2 || dx === -2)
              if (this.chessBoard[newX + (dx === 2 ? -1 : 1)][newY]) continue;
          //nao pode mover um peao pra frente se tiver alguem na frente
            if((dx === 1 || dx === -1) && dy === 0 && newPiece) continue;

            //nao pode mover peao na diagnonal se nao tiver peça, ou se a peça for da mesma cor
